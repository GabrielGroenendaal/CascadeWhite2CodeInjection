...
extern "C"
{
    ...
    // Here is where we put all our shenanigans
    ...
    void THUMB_BRANCH_SaveBtlFieldStatus(BattleFieldStatus *status, void *gameData, void *field)
    {
        ...
        if (PlayerStateZoneID == 446 || PlayerStateZoneID == 445 || PlayerStateZoneID == 385 || PlayerStateZoneID == 551 || PlayerStateZoneID == 383 || PlayerStateZoneID == 329)
        {
            // k::Printf("Appleseeauce");
            PlayerState *playerstate = GameData_GetPlayerState(gameData);
            VecFx32 *vec = PlayerState_GetWPos(playerstate);
            VecFx32 *print;
            // vecfx_normalize(vec, print);
