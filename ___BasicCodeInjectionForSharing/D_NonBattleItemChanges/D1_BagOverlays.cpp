...
extern "C"
{
    ...
    */
    void THUMB_BRANCH_PokeList_ItemUseMain(PokeList *a1)
    {
        ...
        if (ItemUseType <= 27)
        {
            ...
            else if (IsEqual(ItemUseType, 7) || IsEqual(ItemUseType, 8))
            {
                ...
                if (v6 < 4 && !getTMSetting() && !isItemHM(v2->ItemID))
                {
                    PokeList_SubItem(a1, a1->field_28C->ItemID);
                }
                return;
    ...
    /*
        ...
        --------------------------------------------------------------------------------------------------
        --------------------------------- CHECKS POKEMONS FORMS  -----------------------------------------
        --------------------------------------------------------------------------------------------------

        Used for evolutions by stone for pokemon with multiple forms.
    */
    ...
    /*
        ...
        Used to support evolution methods for different forms and reusable Rare Candy / Unusual Candy
    ...
    void THUMB_BRANCH_sub_219B30C(PokeList *a1)
    {
        ...
        else if (IsEqual(ItemUseType, 5))
        {
            ...
            if (PokeList_CanItemWithBattleStatsBeUsed(a1->SelectedPkm, a1->field_28C->ItemID, 0, a1->heapId))
            {
                ...
                if (v5 != RESTORETYPE_RARE_CANDY && v5 != RESTORETYPE_EV_DEF)
                {
                    PokeList_SubItem(a1, a1->field_28C->ItemID);
                }
                return;
        ...
        {
            // Checks to see what form to evolve a Pokemon into.
