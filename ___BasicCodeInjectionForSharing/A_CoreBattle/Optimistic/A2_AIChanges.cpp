...
#pragma region SwitchInAIHelpers
    ...
    bool CheckIfImmuneAbility(int Type, int MoveID, BattleMon *DefendingMon)
    {
        ...
        if (ability == ABIL006_BULLETPROOF && getMoveFlag(MoveID, FLAG_BOMB))//SEARCH_ARRAY(BulletproofMoves, MoveID))
        {
            return true;
        }
        if (ability == ABIL051_WIND_RIDER && getMoveFlag(MoveID, FLAG_WIND))//SEARCH_ARRAY(WindMoves, MoveID))
        {
            return true;
        }
        return false;
