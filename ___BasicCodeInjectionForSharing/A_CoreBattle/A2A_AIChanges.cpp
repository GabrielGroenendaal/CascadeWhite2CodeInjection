...
#pragma region SimulationDamageHelpers
    ...
    u8 checkNaturePower(ServerFlow* flow){
                        ...
                        do
                        {
                            if (Move_GetPP(MonData, v5))
                            {
                                ...
                                if (PML_MoveIsDamaging(ID) && ID != MOVE515_FINAL_GAMBIT && ID != MOVE120_SELF_DESTRUCT && ID != MOVE153_EXPLOSION)
                                {
                                    ...
                                    {
                                #if TRYING_NEW_OPT
                                        v10 = checkRatio(BattleServer_GetServerFlow(work->mainModule->server), MonData, defendingMonChecked, ID, v10);
                                #else 
