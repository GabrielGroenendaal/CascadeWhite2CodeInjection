...
#pragma endregion 
    /*
        ...
        Used to display unique text for certain trainers when their Pokemon faint.
    */
        ...
        if (!a1->pokeInFlag[ID] && BattleMon_IsFainted(a2))
        {
            ...
            if ((!BtlSetup_IsBattleType((void *)a1->mainModule, 512) || !j_MainModule_PokeIDToClientID(ID)) && a1->thruDeadMsgPokeID != ID)
            {
                ...
                if (a1->mainModule->btlSetup->btlType != 0)
                {
                    ...
                    /* Check to see if we're fighting any of the Guardians of Unova and if the Guardian has fallen */
    ...
    */
    ...
    /*
        ...
        --------------------------------------------------------------------------------------------------
        -------------------------------- SUBSEQUENT SWITCH IN LOGIC --------------------------------------
        --------------------------------------------------------------------------------------------------

        Handles Switches After the Initial Switch in
    */
