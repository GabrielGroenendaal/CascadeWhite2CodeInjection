#include "definitions/D2_Definitions.h"
...
#define INCLUDING_DEBUG_SUPPORT false 
...
#pragma region WildPokemonModifiers
    ...
    */
    int THUMB_BRANCH_SAFESTACK_s010F_PokePartyAddEgg(void *vm, void *env)
    {
        ...
        if (Capacity > (int)PokeParty_GetPkmCount(pPartyBlk))
        {
            ...
            u8 hiddenAbilityAllowed = (PML_PersonalGetParam(v13, Personal_SpecialTutor1) >> 16) & 0xFF;\
            ...
            if (hiddenAbilityAllowed > 1)
            {
                ...
                if (random2 <= (25u * hiddenAbilityAllowed))
                {
    ...
    */
    ...
    int THUMB_BRANCH_FieldEncount_CreateWildPkm(PartyPkm *pPkm, EncountManager *mgr, WildPokemonParam *pkmData)
    {
        ...
        if (hiddenAbilityAllowed > 1)
        {
            ...
            if (random2 <= (25u * hiddenAbilityAllowed)) PokeParty_SetHiddenAbil(pPkm, pkmData->Species, pkmData->Forme);
        }
    ...
    */
    ...
    void THUMB_BRANCH_EventFieldTrade_CreatePkm(GameData *gameData, HeapID heapId, PartyPkm *pkm, FieldTradePkm *tradePkm)
    {
        ...
        // CLAUDE: NEW STUFF HERE 
        personal = PML_PersonalLoad(tradePkm->Species, tradePkm->Forme, heapId);
        u8 hiddenAbilityAllowed = (PML_PersonalGetParam(personal, Personal_SpecialTutor1) >> 16) & 0xFF;
        if (hiddenAbilityAllowed > 1)
        {
            int random2 = GFL_RandomLCAlt(1000u);
        ...
        improveIVs(pkm);
        if (tradePkm->IvHP != 255) PokeParty_SetParam(pkm, PF_IvHP, tradePkm->IvHP);
        if (tradePkm->IvATK != 255) PokeParty_SetParam(pkm, PF_IvATK, tradePkm->IvATK);
        if (tradePkm->IvDEF != 255)  PokeParty_SetParam(pkm, PF_IvDEF, tradePkm->IvDEF);
        if (tradePkm->IvSPE != 255)  PokeParty_SetParam(pkm, PF_IvSPE, tradePkm->IvSPE);
        if (tradePkm->IvSPA != 255)  PokeParty_SetParam(pkm, PF_IvSPA, tradePkm->IvSPA);
        if (tradePkm->IvSPD != 255) PokeParty_SetParam(pkm, PF_IvSPD, tradePkm->IvSPD);
        if (tradePkm->Nature != 255) PokeParty_SetParam(pkm, PF_Nature, tradePkm->Nature);
        ...
        {
            u8 numOfPerfectIVs = 0;
            u8 random2 = 0;
            PkmField ivs[6] = {PF_IvHP, PF_IvATK, PF_IvDEF, PF_IvSPA, PF_IvSPD, PF_IvSPE};
            u8 changedIVs[6] = {0, 0, 0, 0, 0, 0};
            do
            {
                random2 = GFL_RandomLCAlt(5u);
                if (changedIVs[random2] == 0)
    ...
    int THUMB_BRANCH_s00F4_DayCareCalcNewLevel(void *a1, void *a2)
    {
        ...
        hiddenAbilityAllowed = (PML_PersonalGetParam(personal, Personal_SpecialTutor1) >> 16) & 0xFF;
        ...
        else if (Any == 2)
        {
            ...
            else if (hiddenAbilityAllowed < 2)
            {
                *Var = 0;
            }
            else
