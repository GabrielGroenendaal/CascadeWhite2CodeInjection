...
#include "definitions/B2_ZoneIdToBackground.h"
#include "definitions/fs.h"
...
static const char ZONE_BG_PATH[] = "battle_background/zone_map.bin";
static const u32 ZONE_BG_COUNT = 615;
static const u8 USE_VANILLA_BG = 0xFF;
static b32 TryReadExpandedBackground(u16 zoneID, u8 *result) {
    if (zoneID >= ZONE_BG_COUNT){
        return false;
    }
    FSFile file; 
    finit(&file);
    if (!romfs_fopen(&file, ZONE_BG_PATH)){
        return false;
    }
    b32 success = romfs_fgetsize(&file) >= ZONE_BG_COUNT 
        && romfs_fseek(&file, zoneID, IO_SEEK_SET)
        && romfs_fread(&file, result, 1) == 1;
    romfs_fclose(&file);
    return success;
}
...
#pragma region settingupTrainers
    ...
    */
    void THUMB_BRANCH_SAFESTACK_SaveBtlFieldStatus(BattleFieldStatus *status, void *gameData, void *field)
    {
        ...
        // THIS IS THE PART WE CHANGE UP
        if (!GetBackgroundsSetting())
        {
            u8 expandedBackground = USE_VANILLA_BG;
            if (TryReadExpandedBackground(PlayerStateZoneID, &expandedBackground) && expandedBackground != USE_VANILLA_BG){
                status->BattleBGID = expandedBackground;
            }
